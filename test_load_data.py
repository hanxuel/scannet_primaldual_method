...
def build_data_generator(data_path,params,istrain=False):
    ...
    while True:
        np.random.shuffle(idxs) # Shuffle all data samples.
         
        for batch_start_idx in range(0, len(idxs), batch_size): # One epoch iterates once over all scenes.
            batch_end_idx = min(batch_start_idx + batch_size, len(idxs)) # Determine the random scenes for current batch.
            batch_idxs = idxs[batch_start_idx:batch_end_idx]
            # By default, set all voxels to unobserved.
            batch_datacost[:] = 0
            batch_groundtruth[:] = 1.0 / nclasses
            ...
            # Prepare data for random scenes in current batch.
            for i, idx in enumerate(batch_idxs):
                datacost_path = os.path.join(data_path, scene_list[idx],"norm_datacost.npz")
...
python evaluate_test.py --data_dir /cluster/project/infk/hilliges/lectures/mp19/project4 --save_dir ./experiments --model_id model_id --export