...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 15 10:22:11 2019
...
def build_data_generator(scene_path, scene_list_path, params):
    ...
    for i, scene_name in enumerate(scene_list):
        ...
        datacost_path = os.path.join(scene_path, scene_name,
                                     "datacost.npz")
        groundtruth_path = os.path.join(scene_path, scene_name,
                                        "groundtruth.npz")

        if not os.path.exists(datacost_path) \
                or not os.path.exists(groundtruth_path):
