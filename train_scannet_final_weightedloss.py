...
# -*- coding: utf-8 -*-
"""
Created on Wed May  8 20:33:40 2019
...
def eachlabel_classification_accuracy(y_true, y_pred,labels):
    ...
    labels_true = tf.argmax(y_true, axis=-1)
    labels_pred = tf.argmax(y_pred, axis=-1)
    #not_unobserved_mask = tf.any(y_true > 0.5, axis=-1)

    accuracy = []
    count = []
    labels=np.arange(len(labels))
    for label in labels:
        mask = tf.equal(labels_true, label)
...
def weighted_loss(y_true, y_pred, weights, params):
    crossentropy_loss=tf.losses.softmax_cross_entropy(onehot_labels=y_true, logits=y_pred, weights=weights)
        ...
        for level in range(nlevels):
            ...
            with tf.variable_scope("weights{}".format(level)):
                conv_weight_variable(
                        "w1", [3,3,3, nclasses, 3 * nclasses], stddev=0.001)
                conv_weight_variable(
                        "w2", [3,3,3, nclasses, 3 * nclasses], stddev=0.001)
...
def get_voxel_weights(voxelgrid,class_weights):
        ...
        voxel_weights = np.zeros(voxelgrid.shape[:-1],dtype=np.float32)
        containedclass= np.unique(np.argmax(voxelgrid,axis=3))
        for class_i in containedclass:
            voxel_weights[np.argmax(voxelgrid,axis=3) == class_i] = class_weights[class_i]

        #for class_i, weight in enumerate(class_weights):
        #    voxel_weights[np.argmax(voxelgrid,axis=3) == class_i] = weight
...
def build_data_generator(data_path,params,istrain=False):
    ...
    batch_weights=np.empty(
            (batch_size, nrows, ncols, nslices), dtype=np.float32)
    ...
    while True:
        ...
        for batch_start_idx in range(0, len(idxs), batch_size):
            ...
            #datacost = datacost_data["volume"]
                ...
                assert datacost.shape[3] == nclasses
                assert datacost.shape == groundtruth.shape
                print("datacost.shape",datacost.shape)
                ...
                while m==False:
                    ...
                    print(choice)
                    if choice==0 and len(np.where(np.argmax(groundtruth,axis=3)==0)[1])>0:#including ceil
                        row_start = np.random.randint(
                                0, max(datacost.shape[0] - nrows, 0) + 1)
                        col_start = Counter(np.where(np.argmax(groundtruth,axis=3)==0)[1]).most_common(1)[0][0]-ncols+1
                        slice_start = np.random.randint(
                                0, max(datacost.shape[2] - nslices, 0) + 1)                       
                    elif choice==0  and len(np.where(np.argmax(groundtruth,axis=3)==0)[1])==0:
                        continue
                    elif choice==1 and len(np.where(np.argmax(groundtruth,axis=3)==1)[1])>0:#including floor
                        row_start = np.random.randint(
                                0, max(datacost.shape[0] - nrows, 0) + 1)
                        col_start = Counter(np.where(np.argmax(groundtruth,axis=3)==1)[1]).most_common(1)[0][0]
                        slice_start = np.random.randint(
                                0, max(datacost.shape[2] - nslices, 0) + 1)
                    elif choice==1 and len(np.where(np.argmax(groundtruth,axis=3)==1)[1])==0:
                        continue
                    elif choice==2 and len(np.where(np.argmax(groundtruth,axis=3)==2)[1])>0:#including wall
                        furtherchoice=np.random.randint(4)
                        if furtherchoice==0:
                            row_start=np.random.randint(
                                    0, max(datacost.shape[0] - nrows, 0) + 1)
                            col_start=np.random.randint(
                                    0, max(datacost.shape[1] - ncols, 0) + 1)
                            slice_start=np.min(np.where(np.argmax(groundtruth,axis=3)==2)[2])
                        elif furtherchoice==1:
                            row_start=np.random.randint(
                                    0, max(datacost.shape[0] - nrows, 0) + 1)
                            col_start=np.random.randint(
                                    0, max(datacost.shape[1] - ncols, 0) + 1)
                            slice_start=np.max(np.where(np.argmax(groundtruth,axis=3)==2)[2])-nslices+1
                        elif furtherchoice==2:
                            row_start=np.min(np.where(np.argmax(groundtruth,axis=3)==2)[0])
                            col_start=np.random.randint(
                                    0, max(datacost.shape[1] - ncols, 0) + 1)
                            slice_start=np.random.randint(
                                    0, max(datacost.shape[2] - nslices, 0) + 1)
                        elif furtherchoice==3:
                            row_start=np.max(np.where(np.argmax(groundtruth,axis=3)==2)[0])-nrows+1
                            col_start=np.random.randint(
                                    0, max(datacost.shape[1] - ncols, 0) + 1)
                            slice_start=np.random.randint(
                                    0, max(datacost.shape[2] - nslices, 0) + 1)
                    elif choice==2 and len(np.where(np.argmax(groundtruth,axis=3)==2)[1])==0:
                        continue    
                    else: #random choose inside                           
                        row_start = np.random.randint(
                                0, max(datacost.shape[0] - nrows, 0) + 1)
                        col_start = np.random.randint(
                                0, max(datacost.shape[1] - ncols, 0) + 1)
                    ...
                    print(row_start,row_end,col_start,col_end,slice_start,slice_end) 
                    if is_sample_valid(groundtruth=groundtruth[row_start:row_end,col_start:col_end,slice_start:slice_end],\
                        ...
                        if istrain==True:
                            voxel_weights=get_voxel_weights(
                                    groundtruth[row_start:row_end,col_start:col_end,slice_start:slice_end],class_weights)
                        else:
                            voxel_weights=np.ones([row_end-row_start,col_end-col_start,slice_end-slice_start])
                        #print("voxel_weights:",voxel_weights.shape)
                        ...
                        batch_weights[i,
                                      :row_end-row_start,
                                      :col_end-col_start,
                                      :slice_end-slice_start] = voxel_weights
                        m=True
                ...
                if num_rot90 > 0:
                    ...
                    batch_groundtruth[i] = np.rot90(batch_groundtruth[i],
                                                    k=num_rot90,
                                                    axes=(0, 1))
                    batch_weights[i] = np.rot90(batch_weights[i],
                                                k=num_rot90,
                                                axes=(0, 1))
                ...
                if flip_axis == 0 or flip_axis == 1:
                    ...
                    batch_groundtruth[i] = np.flip(batch_groundtruth[i],
                                                   axis=flip_axis)
                    batch_weights[i] = np.flip(batch_weights[i],
                                               axis=flip_axis)

            yield (batch_datacost[:len(batch_idxs)],
                   batch_groundtruth[:len(batch_idxs)],
                   batch_weights[:len(batch_idxs)])
...
def train_model(data_path,val_path, model_path, params,labels):
    ...
    train_data_generator = \
        build_data_generator(data_path, params,istrain=True)
    #class_weights_data=np.load(os.path.join(data_path,"class_weights.npz"))
    #class_weights=class_weights_data["class_weights"]
    ...
    val_data_generator = \
        build_data_generator(val_path, val_params,istrain=True)
    ...
    class_weights = tf.placeholder(tf.float32, probs[0].shape[:,-1], name="class_weights")
    ...
    loss_op = weighted_loss(groundtruth, probs[0], class_weights, params)
    ...
    with sess.as_default():
        ...
        for epoch in range(params["nepochs"]):
            ...
            while True:
                ...
                datacost_batch, groundtruth_batch, weights_batch = train_data
                print(datacost_batch.shape)
                print(datacost_batch.dtype)
                print(weights_batch.shape)
                print(weights_batch.dtype)
                ...
                feed_dict[class_weights] = weights_batch
                ...
                train_countlabel_values.append(countlabel)
            ...
            train_eachlable_accuracy_value = \
                np.nanmean(train_eachlabel_accuracy_values,axis=0) #shape(38,)
            train_eachlable_accuracy_value[np.isnan(train_eachlable_accuracy_value)] = 0.0
            print("train_eachlable_accuracy_value.shape",train_eachlable_accuracy_value.shape)
            val_loss_values = []
            val_freespace_accuracy_values = []
            val_occupied_accuracy_values = []
            val_semantic_accuracy_values = []
            val_eachlabel_accuracy_values=[]
            val_countlabel_values=[]

            for _ in range(val_nbatches):
                datacost_batch, groundtruth_batch, weights_batch= next(val_data_generator)
                print(datacost_batch.shape)
                print(datacost_batch.dtype)
                print(weights_batch.shape)
                print(weights_batch.dtype)
                ...
                val_countlabel_values.append(countlabel)
            val_loss_value = \
            ...
            val_eachlable_accuracy_value = \
                np.nanmean(val_eachlabel_accuracy_values,axis=0) #shape(38,)
            val_eachlable_accuracy_value[np.isnan(val_eachlable_accuracy_value)] = 0.0
            print("val_eachlable_accuracy_value.shape",val_eachlable_accuracy_value.shape)
            print(78 * "#")
            print()
...
def parse_args():
    ...
    parser.add_argument("--scene_train_path", default="/cluster/scratch/haliang/SUNCG/origintsdf_trainceil18_wall12")
    parser.add_argument("--scene_val_path", default="/cluster/scratch/haliang/SUNCG/origintsdf_valceil20_wall15")
    ...
    parser.add_argument("--val_nbatches", type=int, default=1)
...
def main():
    ...
    Suncg_LABELS = tuple(label for label, _ in Suncg_labels_color)
    #Suncg_COLORS = tuple(color for _, color in Suncg_labels_color)

    train_model(args.scene_train_path, args.scene_val_path, args.model_path, params,Suncg_LABELS)
