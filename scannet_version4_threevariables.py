...
@author: hliang
"""
#new dataset niter 15, three level; have two projectors; have three levels' weights
#change loss/ loss-weight 2--->10
...
def update_dual(u_, m, iter):
    ...
    with tf.name_scope("dual_update"):
        ...
        with tf.variable_scope("step{}".format(iter), reuse=True):
            sig = tf.get_variable("sig")
        grad_u = conv3d(u_[-1], w1)
        grad_u = sig * grad_u
        update = tf.concat([m[-1],grad_u],axis=-1)
        #print("update_dim for u_",tf.shape(update))
            #p3 = tf.get_variable("p3")
        if iter<5:
            with tf.variable_scope("dual_proxmitor_step5",reuse=True):
                p1 = tf.get_variable("p1") #  pp1 shape [*,*,*,2*nclasses+nclasses+nclasses,*]
            ...
            with tf.variable_scope("dual_proxmitor_step10",reuse=True):
                p1 = tf.get_variable("p1") #  pp1 shape [*,*,*,2*nclasses+nclasses+nclasses,*]
                p2 = tf.get_variable("p2") #
                d_pro1 = tf.get_variable("d_pro1")
                d_pro2 = tf.get_variable("d_pro2")
            ...
            with tf.variable_scope("dual_proxmitor_step15",reuse=True):
                p1 = tf.get_variable("p1") #  pp1 shape [*,*,*,2*nclasses+nclasses+nclasses,*]
                p2 = tf.get_variable("p2") #  
                d_pro1 = tf.get_variable("d_pro1")
                d_pro2 = tf.get_variable("d_pro2")
        update = Relu(conv3d(update, p1)+d_pro1) # p1 shape:[*,*,*,6*nclasses,*]
        #update = Relu(conv3d(update, p2)) # p2 shape:[*,*,*,*,*]
        update = conv3d(update, p2)+ d_pro2 # p3 shape:[*,*,*,*,3*nclasses]
        #m[-1] = update
        m[-1] = m[-1] + update
        #m = m + sig * update
...
def update_primal(u, u_, m, l, d, level, iter):
    assert len(u) == len(m)
    assert len(u) == len(l)
    #assert len(u) == len(d)
    ...
    with tf.name_scope("primal_update"):
        ...
        div_m = conv3d_adj(m[-1], w1, nclasses) #nclasses
        
        d_term = d[len(d)-level-1]+l_rshp #nclasses
        update = -tau * tf.concat([div_m,d_term],axis=-1) #
        
        primal = tf.concat([u[-1],u_[-1]],axis=-1) #2*nclasses
        update = tf.concat([primal,update],axis=-1) #4*nclasses
        print("shape of update:",tf.shape(update))
        if iter<5:
            with tf.variable_scope("primal_proxmitor_step5",reuse=True):
            ...
            with tf.variable_scope("primal_proxmitor_step10",reuse=True):
                pp1 = tf.get_variable("pp1") #  pp1 shape [*,*,*,2*nclasses+nclasses+nclasses,*]
                pp2 = tf.get_variable("pp2") #
                p_pro1 = tf.get_variable("p_pro1")
                p_pro2 = tf.get_variable("p_pro2")
            ...
            with tf.variable_scope("primal_proxmitor_step15",reuse=True):
                pp1 = tf.get_variable("pp1") #  pp1 shape [*,*,*,2*nclasses+nclasses+nclasses,*]
                pp2 = tf.get_variable("pp2") #
                p_pro1 = tf.get_variable("p_pro1")
                p_pro2 = tf.get_variable("p_pro2")
            #pp3 = tf.get_variable("pp3") # pp3 shape [*,*,*,*,2*nclasses]
        update = Relu(conv3d(update, pp1)+p_pro1) 
        #update = Relu(conv3d(update, pp2))
        update = conv3d(update, pp2)+p_pro2
        
        primal += update
        #primal = tf.minimum(1.0, tf.maximum(primal, 0.0))
        u_[-1] = tf.minimum(-1.0, tf.maximum(primal[...,nclasses:], 2.0))
        u[-1]  = tf.minimum(1.0, tf.maximum(primal[...,:nclasses], 0.0))
...
def build_model(params):
    ...
    with tf.device('/gpu:' + str(0)):
        ...
        with tf.variable_scope("weights_step5"):
            conv_weight_variable(
                    "w1", [2,2,2, nclasses, 3 * nclasses], stddev=0.001)
        with tf.variable_scope("weights_step10"):
            conv_weight_variable(
                    "w1", [2,2,2, nclasses, 3 * nclasses], stddev=0.001)
        with tf.variable_scope("weights_step15"):
            conv_weight_variable(
                    "w1", [2,2,2, nclasses, 3 * nclasses], stddev=0.001)

            #conv_weight_variable(
                #        "w2", [2,2,2, nclasses, 3 * nclasses], stddev=0.001)
        with tf.variable_scope("dual_proxmitor_step5"):
            conv_weight_variable(
                    "p1", [2,2,2, 6*nclasses, 6 * nclasses], stddev=0.001)
            conv_weight_variable(
                    "p2", [2,2,2, 6*nclasses, 3*nclasses], stddev=0.001)
            d_pro1 = bias_weight_variable("d_pro1", [6*nclasses])
            d_pro2 = bias_weight_variable("d_pro2", [3*nclasses])
        with tf.variable_scope("dual_proxmitor_step10"):
            conv_weight_variable(
                    "p1", [2,2,2, 6*nclasses, 6 * nclasses], stddev=0.001)
            conv_weight_variable(
                    "p2", [2,2,2, 6*nclasses, 3*nclasses], stddev=0.001)
        ...
        with tf.variable_scope("dual_proxmitor_step15"):
            conv_weight_variable(
                    "p1", [2,2,2, 6*nclasses, 6 * nclasses], stddev=0.001)
            conv_weight_variable(
                    "p2", [2,2,2, 6*nclasses, 3*nclasses], stddev=0.001)
        ...
        with tf.variable_scope("primal_proxmitor_step5"):
            conv_weight_variable(
                    "pp1", [2,2,2,4 * nclasses, 4 * nclasses], stddev=0.001)
            conv_weight_variable(
                    "pp2", [2,2,2,4 * nclasses, 2 * nclasses], stddev=0.001)
            p_pro1 = bias_weight_variable("p_pro1", [4*nclasses])
            p_pro2 = bias_weight_variable("p_pro2", [2*nclasses])
        with tf.variable_scope("primal_proxmitor_step10"):
            conv_weight_variable(
                    "pp1", [2,2,2,4 * nclasses, 4 * nclasses], stddev=0.001)
            conv_weight_variable(
                    "pp2", [2,2,2,4 * nclasses, 2 * nclasses], stddev=0.001)
            p_pro1 = bias_weight_variable("p_pro1", [4*nclasses])
            p_pro2 = bias_weight_variable("p_pro2", [2*nclasses])
        with tf.variable_scope("primal_proxmitor_step15"):
            conv_weight_variable(
                    "pp1", [2,2,2,4 * nclasses, 4 * nclasses], stddev=0.001)
            conv_weight_variable(
                    "pp2", [2,2,2,4 * nclasses, 2 * nclasses], stddev=0.001)
...
def train_model(data_path,val_path, model_path, params):
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        groundtruth_level0 = tf.placeholder(tf.float32, probs[0].shape, name="groundtruth_level0")
        groundtruth_level1 = tf.placeholder(tf.float32, probs[1].shape, name="groundtruth_level1")
        u_init = []
        u_init_ = []
        m_init = []
        l_init = []
        ...
        loss_op_level0 = categorical_crossentropy(groundtruth_level0, probs[0], params)
        loss_op_level1 = categorical_crossentropy(groundtruth_level1, probs[1], params)
        loss_op = 0.2*loss_op_level0 + 0.5*loss_op_level1 + loss_op
        ...
        #train_saver.restore(sess, os.path.join(checkpoint_path, "checkpoint-00000011"))
        ...
        for epoch in range(params["nepochs"]):
            ...
            while True:
                ...
                BSZ = (2,2,2)
                m1,n1,q1,p1 = groundtruth_batch.shape[1:]
                groundtruth_level1_batch=groundtruth_batch.reshape(num_batch_samples,\
                                                                   m1//BSZ[0],BSZ[0],n1//BSZ[1],BSZ[1],q1//BSZ[2],BSZ[2],p1).mean(axis=(2,4,6))
                print(groundtruth_level1_batch.shape)
                m0,n0,q0,p0 = groundtruth_level1_batch.shape[1:]
                groundtruth_level0_batch=groundtruth_level1_batch.reshape(num_batch_samples,\
                                                                   m0//BSZ[0],BSZ[0],n0//BSZ[1],BSZ[1],q0//BSZ[2],BSZ[2],p0).mean(axis=(2,4,6))
                ...
                feed_dict[groundtruth_level0] = groundtruth_level0_batch
                feed_dict[groundtruth_level1] = groundtruth_level1_batch
