...
@author: hliang
"""
#new dataset niter 15, three level; have two projectors
#lossweight5--->10
import os
import glob
import argparse
...
def categorical_crossentropy(y_true, y_pred, params):
    ...
    with tf.name_scope("categorical_cross_entropy"):
        ...
        unweighted_cross_entropy = weights * y_true * tf.log(y_pred)
        occupied_cross_entropy = \
            -tf.reduce_sum(tf.boolean_mask(unweighted_cross_entropy, occupied_mask)) / \
        ...
        for iter in range(niter):
            ...
            with tf.variable_scope("weights_step{}".format(iter)):
                conv_weight_variable(
                        "w1", [2,2,2, nclasses, 3 * nclasses], stddev=0.001) #filter size and kernal size can be changed
                #conv_weight_variable(
...
def parse_args():
    ...
    parser.add_argument("--loss_weight", type=float, default=5) #5
