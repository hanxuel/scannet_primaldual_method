    ...
    with tf.device('/gpu:' + str(1)):
        ...
        for level in range(nlevels):
            for iter in range(nlevels):
                u_loop,  m_loop, l_loop = primal_dual(
                        u_loop,  m_loop, l_loop, d_encoded,level, iter)
...
def train_model(data_path,val_path, model_path, params):
    ...
    with tf.Session(config=tf_config) as sess:
        probs, datacost, u, m, l = build_model(params)
        memory,cpu=getMemCpu()
        time.sleep(0.2)
