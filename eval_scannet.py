...
def eval_model(checkpoint_path, datacost_path, params):
    ...
    with tf.Session() as sess:
        ...
        d = graph.get_tensor_by_name("reconstruction/d:0")
        p = graph.get_tensor_by_name("reconstruction/probs0:0")
        ...
        l_final = []
        for level in range(nlevels):
            u.append(graph.get_tensor_by_name("reconstruction/u{}:0".format(level)))
            u_.append(graph.get_tensor_by_name("reconstruction/u_{}:0".format(level)))
            m.append(graph.get_tensor_by_name("reconstruction/m{}:0".format(level)))
            l.append(graph.get_tensor_by_name("reconstruction/l{}:0".format(level)))
            u_final.append(graph.get_tensor_by_name("reconstruction/u_final{}:0".format(level)))
            u_final_.append(graph.get_tensor_by_name("reconstruction/u_final_{}:0".format(level)))
            m_final.append(graph.get_tensor_by_name("reconstruction/m_final{}:0".format(level)))
            l_final.append(graph.get_tensor_by_name("reconstruction/l_final{}:0".format(level)))
...
def parse_args():
    ...
    parser.add_argument("--checkpoint_path", default="/cluster/home/haliang/Scannet_Data_Code/learned-semantic3d-master/r20_10/changeloss_from0/checkpoint")
    parser.add_argument("--datacost_path", default="/home/hliang/Downloads/SUNCG_Data/val_ceil5_wall_5")
    parser.add_argument("--output_path", default="/home/hliang/Downloads/SUNCG_Data/val_ceil5_wall_5/scene79")
