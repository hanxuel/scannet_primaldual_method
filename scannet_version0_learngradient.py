...
@author: hliang
"""
#new dataset niter 17*3, three level,each level share same weights;
#all steps are different
...
def update_dual(u_, m, iter,level):
    ...
    with tf.name_scope("dual_update"):
        ...
        with tf.variable_scope("dual_proxmitor_level{}".format(level),reuse=True):
...
def update_primal(u, u_, m, l, d, level, iter):
    ...
    with tf.name_scope("primal_update"):
        ...
        with tf.variable_scope("primal_proxmitor_level{}".format(level),reuse=True):
        ...
        update3 = conv3d(update2, pp3)+p_pro3
        primal += update3
...
def build_model(params):
    ...
    with tf.device('/gpu:' + str(0)):
        ...
        for level in range(nlevels):
            ...
            with tf.variable_scope("dual_proxmitor_level{}".format(level)):
                conv_weight_variable(
                ...
                #conv_weight_variable(
                #        "p3", [2,2,2, 6*nclasses, 3 * nclasses], stddev=0.001)
            with tf.variable_scope("primal_proxmitor_level{}".format(level)):
                conv_weight_variable(
                        "pp1", [2,2,2,3 * nclasses, 3 * nclasses], stddev=0.001)
                conv_weight_variable(
                        "pp2", [2,2,2,3 * nclasses, 3 * nclasses], stddev=0.001)
                conv_weight_variable(
                        "pp3", [2,2,2,3 * nclasses, 2 * nclasses], stddev=0.001)
                p_pro1 = bias_weight_variable("p_pro1", [3*nclasses])
                p_pro2 = bias_weight_variable("p_pro2", [3*nclasses])
                p_pro3 = bias_weight_variable("p_pro3", [2*nclasses])
...
def parse_args():
    ...
    parser.add_argument("--niter", type=int, default=17)
    ...
    parser.add_argument("--loss_weight", type=float, default=6) #5
