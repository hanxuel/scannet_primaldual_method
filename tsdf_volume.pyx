...
cdef class TSDFVolume:
    ...
    #cdef float resolution_x
    #cdef float resolution_y
    #cdef float resolution_z
    cdef float max_distance
    cdef float[:, :, :, ::1] volume
    cdef int[::1] size
    cdef float resolution

    def __init__(self, num_labels, size, bbox, resolution_factor,free_space_vote=0.5, occupied_space_vote=1):
        assert num_labels > 0
        #assert resolution > 0
        ...
        self.resolution = np.mean(np.true_divide(volume_size.ravel(),np.array(size)))
        #self.resolution_x=resolution_x
        #self.resolution_y=resolution_y
        #self.resolution_z=resolution_z
        self.max_distance = resolution_factor * self.resolution
        volume_shape = np.floor(size).astype(np.int32).tolist()
        self.volume = np.zeros(volume_shape + [num_labels + 1],dtype=np.float32)
        #self.labels = np.zeros(volume_shape,dtype=np.int32)
            ...
            for j in range(self.volume.shape[1]):
                ...
                for k in range(self.volume.shape[2]):
                    ...
                    if abs(signed_distance) > self.max_distance:
                        ...
                        continue
###why minus free_space_vote; should times signed_distance?
...
cdef class TSDFVolume:
    ...
    def __init__(self, num_labels, bbox, resolution, resolution_factor,
        ...
        volume_shape = np.floor(volume_shape).astype(np.int32).tolist()
