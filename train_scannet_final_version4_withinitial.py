...
def read_groundtruth(room_path, numclasses):
    ...
    # Read the dat file
    #groundtruth contains label's groundtruth
...
def update_dual(u_, m, iter):
    ...
    with tf.name_scope("dual_update"):
        ...
        update = conv3d(update, p2) # p3 shape:[*,*,*,*,3*nclasses]
        #m[-1] = update
        m[-1] = m[-1] + update
        #m = m + sig * update
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for epoch in range(params["nepochs"]):
            ...
            while True:
                ...
                BSZ1 = (4,4,4)
                m2,n2,q2,p2 = datacost_batch.shape[1:]
                datacost_level0_batch=datacost_batch.reshape(num_batch_samples,\
                                                             m2//BSZ1[0],BSZ1[0],n2//BSZ1[1],BSZ1[1],q2//BSZ1[2],BSZ1[2],p2).mean(axis=(2,4,6))
                for i in range(num_batch_samples):
                    init_label=np.argmin(datacost_level0_batch[i],axis=-1)
                    xres,yres,zres = np.shape(init_label)
                    init_label = np.reshape(init_label, init_label.size)
                    init_label_onehot = np.zeros((init_label.size, nclasses), dtype=np.float32)
                    init_label_onehot[np.arange(init_label.size), init_label] = 1.0
                    init_label_onehot = np.reshape(init_label_onehot, [xres, yres, zres, nclasses])
                    index_x=np.array(np.where(np.count_nonzero(datacost_level0_batch[i],axis=-1)==0))[0]
                    index_y=np.array(np.where(np.count_nonzero(datacost_level0_batch[i],axis=-1)==0))[1]
                    index_z=np.array(np.where(np.count_nonzero(datacost_level0_batch[i],axis=-1)==0))[2]
                    init_label_onehot[index_x,index_y,index_z,:]=1.0/nclasses
                    u_init[0][i] = init_label_onehot
                    u_init_[0][i] = init_label_onehot
...
python train.py --data_dir /cluster/project/infk/hilliges/lectures/mp19/project4 --save_dir ./experiments --experiment_name 50weighted
