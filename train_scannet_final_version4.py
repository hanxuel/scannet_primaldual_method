...
@author: hliang
"""
## different weight at different itereation; 
#defferent stepsize for different iteration; 
#add channels for dual variable
import os
import glob
import argparse
import numpy as np
    ...
    for level in range(nlevels):
        ...
        with tf.variable_scope("weights_step10_{}".format(level)):
            conv_weight_variable(
                    "w1", [2,2,2, nclasses, 12 * nclasses], stddev=0.001)
            conv_weight_variable(
                    "w2", [2,2,2, nclasses, 12 * nclasses], stddev=0.001)
            ...
            #datacost = datacost_data["volume"]
                ...
                while m==False:
                    ...
                    if choice==0  and len(np.where(np.argmax(groundtruth,axis=3)==0)[1])>0:#including ceil
                        ...
                        col_start = max(0, Counter(np.where(np.argmax(groundtruth,axis=3)==0)[1]).most_common(1)[0][0]-ncols+1)
