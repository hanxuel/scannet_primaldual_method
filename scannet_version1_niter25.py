...
"""
#new dataset
#niter 25 every 5 share same weights; iterstep as variables
import os
import glob
import argparse
import numpy as np
...
def build_model(params):
        ...
        for level in range(nlevels):
            ...
            l.append(tf.placeholder(
                         tf.float32, [None, nrows_level, ncols_level,
                                      nslices_level],
                                      name="l{}".format(level)))
            with tf.variable_scope("weights_step5_{}".format(level)):
            ...
            with tf.variable_scope("weights_step10_{}".format(level)):
                ...
                conv_weight_variable(
                        "w2", [2,2,2, nclasses, 3 * nclasses], stddev=0.001)
            with tf.variable_scope("weights_step15_{}".format(level)):
                conv_weight_variable(
                        "w1", [2,2,2, nclasses, 3 * nclasses], stddev=0.001)
                conv_weight_variable(
                        "w2", [2,2,2, nclasses, 3 * nclasses], stddev=0.001)
            with tf.variable_scope("weights_step20_{}".format(level)):
                ...
                conv_weight_variable(
                        "w2", [2,2,2, nclasses, 3 * nclasses], stddev=0.001)
            with tf.variable_scope("weights_step25_{}".format(level)):
                conv_weight_variable(
                        "w1", [2,2,2, nclasses, 3 * nclasses], stddev=0.001)
                conv_weight_variable(
                        "w2", [2,2,2, nclasses, 3 * nclasses], stddev=0.001)
    ...
    # Create a copy of the placeholders for the loop variables.
        ...
        for iter in range(5):
            u_loop, u_loop_, m_loop, l_loop = primal_dual(
                    u_loop, u_loop_, m_loop, l_loop, d_encoded, 5, iter)
        for iter in range(5,10,1):
            u_loop, u_loop_, m_loop, l_loop = primal_dual(
                    u_loop, u_loop_, m_loop, l_loop, d_encoded, 10, iter)
        for iter in range(10,15,1):
            u_loop, u_loop_, m_loop, l_loop = primal_dual(
                    u_loop, u_loop_, m_loop, l_loop, d_encoded, 15, iter)
    with tf.device('/gpu:' + str(1)):
        for iter in range(15,20,1):
            u_loop, u_loop_, m_loop, l_loop = primal_dual(
                    u_loop, u_loop_, m_loop, l_loop, d_encoded, 20, iter)
        for iter in range(20,25,1):
            u_loop, u_loop_, m_loop, l_loop = primal_dual(
                    u_loop, u_loop_, m_loop, l_loop, d_encoded, 25, iter)
...
def train_model(data_path,val_path, model_path, params):
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        train_saver.restore(sess, os.path.join(checkpoint_path, "checkpoint-00000009"))
        print("checkpoint-00000009 has been restored")
       
        for epoch in range(14,params["nepochs"]):
